GET_USER_ACTIVE_BRAWL = f'''SELECT * FROM brawls WHERE host_id = %s OR opponent_id = %s AND status != "inactive"'''
ADD_BRAWL = f'''INSERT INTO brawls VALUES (%s, 0, %s, %s,0, 0, "open", %s, "{{}}", 0, %s)'''
#               host, opp, title, terms, start, length, status, brawl_pot, voters, winner, image
...
SET_BRAWL_OPPONENT = f'''UPDATE brawls SET opponent_id = %s WHERE host_id = %s AND status != "inactive"'''
...
UPDATE_BRAWL_FOR_START = f'''UPDATE brawls SET brawl_pot = %s, voters = %s, start = %s WHERE host_id = %s AND status != "inactive"'''
UPDATE_BRAWL_FOR_END = f'''UPDATE brawls SET winner = %s, length = %s, status = "inactive" WHERE host_id = %s AND status != "inactive"'''
DELETE_BRAWL = f'''DELETE FROM brawls WHERE host_id = %s AND status != "inactive"'''
