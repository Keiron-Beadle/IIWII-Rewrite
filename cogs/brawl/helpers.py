...
def get_brawl_channel(guild_id : int):
...
async def guild_has_brawl_channel(interaction : discord.Interaction):
    return get_brawl_channel(interaction.guild.id) != 0

async def on_start_brawl(interaction : discord.Interaction, brawl_channel, pot, image_link, attachment_image):
    ...
    modal = views.BrawlRequestModal(request, brawl_channel)
